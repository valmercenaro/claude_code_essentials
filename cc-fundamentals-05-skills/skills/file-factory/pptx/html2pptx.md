...
**Template**: Use [`templates/safe-margins-16x9.html`](templates/safe-margins-16x9.html) as a starting point - it has pre-validated margins (0.5" from edges, 45pt bottom margin) to prevent overflow errors.
...
// Then add via pptxgenjs (see Background Images section below)
```

### Background Images

**CRITICAL**: Never use CSS `background-image: url()` in HTML slides - paths get corrupted during processing (double prefixes like `C:\C:\` and URL-encoded spaces).

**Correct approach**: Add backgrounds via pptxgenjs BEFORE calling html2pptx:
...
// 1. Create slide first
const slide = pptx.addSlide();

// 2. Add background image (covers full 16:9 slide)
slide.addImage({
  path: 'background.png',
  x: 0,
  y: 0,
  w: 10,      // Full width in inches
  h: 5.625    // Full height for 16:9 aspect
});

// 3. Then call html2pptx with existing slide
const { placeholders } = await html2pptx('slide.html', pptx, { slide });

// 4. Add any charts/content to placeholders
// ...

await pptx.writeFile('output.pptx');
```

**Why this works**: pptxgenjs handles file paths correctly, while html2pptx's CSS path processing has known issues with Windows paths and special characters.
...
## Validated HTML Component Templates

Use these pre-validated patterns to avoid common validation errors:

### Number Badges
```html
<!-- ✅ CORRECT: Number in <p> tag inside styled div -->
...
.num p { margin: 0; font-size: 14pt; }
</style>

<!-- ❌ WRONG: Text directly in div - will NOT appear -->
<div class="num">1</div>
```

### Icon Badges (e.g., "CC", "TT", "OC")
```html
<!-- ✅ CORRECT -->
<div class="icon"><p>CC</p></div>
<style>
.icon {
  background: #3B82F6;
  color: #FFFFFF;
...
.icon p { margin: 0; font-size: 10pt; font-weight: bold; }
</style>
...
### Labels with Backgrounds (e.g., ".pdf", "SECTION 2")
```html
<!-- ✅ CORRECT -->
<div class="label"><p>.pdf</p></div>
<style>
.label {
  background: #4B5563;
  padding: 2pt 5pt;
...
.label p { margin: 0; color: #D1D5DB; font-size: 8pt; }
</style>

<!-- ❌ WRONG: <p> with background - validation error -->
<p class="label">.pdf</p>
```

### Tags/Pills (e.g., model names like "sonnet", "haiku")
```html
<!-- ✅ CORRECT: Background on div, text in p -->
<div class="tag"><p>sonnet</p></div>
<style>
.tag {
...
.tag p { margin: 0; color: #D1D5DB; font-size: 7pt; }
</style>

<!-- ❌ WRONG: <p> with background -->
<p class="tag">sonnet</p>
```

### Rule Summary
| Pattern | Correct | Wrong |
|---------|---------|-------|
| Number badges | `<div class="num"><p>1</p></div>` | `<div class="num">1</div>` |
| Icon text | `<div class="icon"><p>CC</p></div>` | `<div class="icon">CC</div>` |
| Labels | `<div class="label"><p>.pdf</p></div>` | `<p class="label">.pdf</p>` |
| Any styled container | Background on `<div>`, text in `<p>` | Background on `<p>` |
...
**Pre-flight dependency check** (add to scripts to catch missing modules early):

```javascript
// Verify dependencies exist before running
const requiredModules = ['pptxgenjs', 'sharp', 'playwright'];
for (const mod of requiredModules) {
  try {
    require.resolve(mod);
  } catch {
    throw new Error(`Missing dependency: npm install ${mod}`);
  }
}
```
